...
class RecipeManageController extends Controller
{
    public function recipe_list(Request $request)
    {
        ...
        $recipes->when($request->filled('category_id'), fn($q) => $q->where('category_id', $request->category_id));
        $recipes->when($request->filled('protein_id'), fn($q) => $q->where('protein_id', $request->protein_id));
        $recipes->when($request->filled('calory_id'), fn($q) => $q->where('calories_id', $request->calory_id));
        $recipes->when($request->filled('carb_id'), fn($q) => $q->where('carb_id', $request->carb_id));
        $recipes->when($request->filled('cuisine_id'), fn($q) => $q->where('cuisine_id', $request->cuisine_id));
        $recipes->when($request->filled('health_goal_id'), fn($q) => $q->where('health_goal_id', $request->health_goal_id));
        $recipes->when($request->filled('time_to_clock_id'), fn($q) => $q->where('time_to_clock_id', $request->time_to_clock_id));
            ...
            if ($request->filled('search')) {
                ...
                $query->where(function ($q) use ($search) {
                    $q->where('title', 'LIKE', "%$search%")
                        ->orWhereHas('calory', fn($q) => $q->where('name', 'LIKE', "%$search%"))
                        ->orWhereHas('protein', fn($q) => $q->where('name', 'LIKE', "%$search%"))
                        ->orWhereHas('carb', fn($q) => $q->where('name', 'LIKE', "%$search%"))
                        ->orWhereHas('health_goal', fn($q) => $q->where('name', 'LIKE', "%$search%"));
                });
            }
        });
    ...
    public function recipe_details(Request $request, $id)
    {
        ...
        return response()->json([
            ...
            'instructions'          => collect($recipe->instructions)->map(function ($instruction) {
                $instruction['image_url'] = isset($instruction['image_url']) && $instruction['image_url']
                ? url($instruction['image_url'])
                : null;
                return $instruction;
            }),
        ]);
